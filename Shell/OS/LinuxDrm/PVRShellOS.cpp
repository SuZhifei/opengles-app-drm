/******************************************************************************
 ...
 @Platform     DRM support for any Linux
...
struct SDrmFbWrapper *PVRShellInitOS::DrmFbGetFromBo(struct gbm_bo *bo)
{
	...
	if (ret) {
		//m_pShell->PVRShellOutputDebug("failed to create fb: %s\n", strerror(errno));
		delete fb;
		return NULL;
	}
...
bool PVRShellInit::OsInitOS()
{
	...
	/*We could be fancy here by getting drm device from udev*/
	/*Or, user could pass drm device node through CMDLINE	*/
	/*For the time being lets keep calm and use card0	*/
