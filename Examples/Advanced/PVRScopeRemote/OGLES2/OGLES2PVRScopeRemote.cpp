...
bool OGLES2PVRScopeRemote::RenderScene()
{
	if(m_psSPSCommsData)
	{
		m_bCommsOK &= pplSendProcessingBegin(*m_psSPSCommsData, "RenderScene", static_cast<unsigned int>(strlen("RenderScene")), m_i32FrameCounter);
		...
		if(!(m_i32FrameCounter % 100))
		{
			...
			m_bCommsOK &= pplSendMark(*m_psSPSCommsData, buf, nLen);
		}
	...
	*/
	...
	if(m_psSPSCommsData)
	{
		m_bCommsOK &= pplCountersUpdate(*m_psSPSCommsData, m_anCounterReadings);
		m_bCommsOK &= pplSendProcessingEnd(*m_psSPSCommsData);
	}
