...
void CPVRScopeGraph::Ping(const unsigned int nTimeUS)
{
	if(m_pPVRScopeData)
	{
		...
		*/
		if(PVRScopeReadCountersThenSetGroup(m_pPVRScopeData, psReading, nTimeUS, nGroupSelect) && psReading)
		{
			m_nUpdateIntervalCounter = 0;
...
{
	return nCounter < m_nCounterNum && m_pCounters[nCounter].bPercentage;
}
